...
public class AIService
{
    ...
    {
        // Không có danh mục nào để chọn → trả null luôn.
        if (categories == null || categories.Count == 0) return null;

        // Lấy cấu hình AI đang áp dụng (model, tham số...) từ AiSetting.
        var cfg = await GetEffectiveAsync();

        // Tạo danh sách danh mục đánh số "1. Tên\n2. Tên..." để đưa vào prompt cho AI chọn.
        var list = string.Join("\n", categories.Select((c, i) => $"{i + 1}. {c.Name}"));

        // Dựng prompt: yêu cầu AI chọn DUY NHẤT một danh mục, chỉ trả về đúng TÊN, không giải thích.
        var prompt =
            ...
            $"Danh sách danh mục:\n{list}\n\n" +                           // nhét danh sách danh mục vào
            $"Tiêu đề: {title}\nTóm tắt: {summary}\n\nDanh mục phù hợp nhất:"; // nhét tiêu đề + tóm tắt

        // Gọi mô hình AI với prompt. Trả về: text kết quả, số token, tên model, lỗi (nếu có).
        var (text, tokens, model, error) = await CallModelAsync(prompt, cfg);

        // === AI lỗi hoặc trả rỗng → ghi log thất bại rồi trả null (không gán được danh mục) ===
        if (error != null || string.IsNullOrWhiteSpace(text))
        {
            await TryLogAsync(new AILog
            {
                Action = "classify_category",
                PromptText = prompt,          // loại hành động + prompt đã gửi
                IsSuccess = false,
                ErrorMsg = error ?? "empty response",
                ModelUsed = model
        ...
        // Bỏ khoảng trắng thừa quanh câu trả lời của AI (vd "Trí tuệ AI ").
        var reply = text.Trim();

        // Chuẩn hoá về dạng slug (không dấu, thường, gạch nối) để so khớp "mềm", không phụ thuộc hoa/thường/dấu.
        var rn = SlugHelper.MakeSlug(reply);

        // === BƯỚC ƯU TIÊN: tra BẢNG ÁNH XẠ (quy tắc admin/đã học): nhãn AI -> danh mục hệ thống ===
        var maps = await _db.CategoryMappings.ToListAsync();                // nạp tất cả quy tắc ánh xạ
                                                                            // Tìm quy tắc: (a) nhãn khớp CHÍNH XÁC theo slug; nếu không có thì (b) nhãn (>=2 ký tự) nằm TRONG câu trả lời.
        ...
        // Nếu tìm được quy tắc VÀ danh mục đó vẫn tồn tại trong danh sách → dùng luôn quy tắc.
        if (mp != null && categories.Any(c => c.Id == mp.CategoryId))
        {
            // Ghi log thành công, ghi rõ "AI trả X → ánh xạ → danh mục Y".
            await TryLogAsync(new AILog
            {
                Action = "classify_category",
            ...
            return mp.CategoryId;                                           // trả danh mục theo quy tắc, KHÔNG đoán thêm
        }

        // === BƯỚC DỰ PHÒNG: nếu không có quy tắc, khớp trực tiếp câu trả lời với TÊN danh mục ===
        var match = categories.FirstOrDefault(c => SlugHelper.MakeSlug(c.Name) == rn)                     // (1) khớp chính xác
                 ?? categories.FirstOrDefault(c => rn.Contains(SlugHelper.MakeSlug(c.Name)))              // (2) tên danh mục nằm trong câu trả lời
                 ?? categories.FirstOrDefault(c => SlugHelper.MakeSlug(c.Name).Contains(rn) && rn.Length >= 3); // (3) câu trả lời nằm trong tên (>=3 ký tự)

        // Ghi log kết quả cuối (thành công nếu match != null; ghi cả trường hợp "không khớp danh mục").
        await TryLogAsync(new AILog
        {
            Action = "classify_category",
        ...
        // Trả CategoryId của danh mục khớp; null nếu không khớp được cái nào.
        return match?.Id;
    }
